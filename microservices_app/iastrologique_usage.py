...
from microservices_app.utils_iastrologique import create_job, create_server, \
...
logger.info("Storage definition")
...
iastrologique_data_fetch_pg_job = create_job(
    "IAstrologique Data Fetch on Postgres", 10*u.kB, 500*u.kB, 10*u.kB, 1*u.s, 1*u.GB, 1*u.core, postgres_server)
mongodb_data_fetch_job = create_job(
    "MongoDB Data Fetch", 10*u.kB, 500*u.kB, 10*u.kB, 1*u.s, 1*u.GB, 1*u.core, mongodb_server)
form_filling_job = create_job(
    "Form Filling",
    50*u.kB, 2*u.MB, 50*u.kB, 3*u.s, 2*u.GB, 1*u.core, iastrologique_app_server)

five_modeling_result_jobs = create_job(
    "5 modeling result",
    10*u.kB, 2*u.MB, 10*u.kB, 3*u.s, 2*u.GB, 1*u.core, iastrologique_app_server, nb_result_steps_trials)
five_bis_result_api_calls_jobs = create_job(
    "5 bis result API calls",
    0*u.kB, 500*u.kB, 0*u.kB, 1*u.s, 2*u.GB, 1*u.core, eks_server, nb_result_steps_trials)
five_ter_result_postgres_calls_jobs = create_job(
    "5 ter result postgres calls",
    0*u.kB, 500*u.kB, 0*u.kB, 1*u.s, 2*u.GB, 1*u.core, postgres_server ,nb_result_steps_trials)

pdf_retrevial_job = create_job(
    "PDF retrieval", 10*u.kB, 200*u.kB, 10*u.kB, 1*u.s, 2*u.GB, 1*u.core, mongodb_server)
...
tracking_metrics_job = create_job(
    "Tracking metrics", 5*u.MB, 0*u.kB, 5*u.MB, 3*u.s, 1*u.GB, 1*u.core, prometheus_server
)

logger.info("UserJourney Step definition")
...
logger.info("UserJourney definition")
...
logger.info("UsagePattern definition")
...
print(iastrologique_app_server.nb_of_instances)