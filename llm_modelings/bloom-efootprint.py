...
carbon_footprint_fabrication_server_without_gpu = SourceValue(2500 * u.kg, bloom_paper_source, "Carbon footprint without GPU")
...
server = Autoscaling(
    "Training GPU server",
    carbon_footprint_fabrication=(carbon_footprint_fabrication_server_without_gpu + NB_OF_GPUs * carbon_footprint_fabrication_one_gpu).set_label("default"),
...
bloom_training_step = UserJourneyStep(
    "Bloom training", service, SourceValue(1 * u.TB / u.uj, Sources.USER_DATA),
    SourceValue(1 * u.TB / u.uj, Sources.USER_DATA),
    user_time_spent=SourceValue(24 * u.h / u.uj, Sources.USER_DATA),
...
server_inference = Autoscaling(
    ...
    carbon_footprint_fabrication=(carbon_footprint_fabrication_server_without_gpu + GPU_INFERENCE_NB * carbon_footprint_fabrication_one_gpu).set_label("default"),
...
bloom_inference_step = UserJourneyStep(
    "Bloom inference", service_inference, SourceValue(100 * u.kB / u.uj, Sources.USER_DATA),
    SourceValue(100 * u.kB / u.uj, Sources.USER_DATA),
    user_time_spent=request_duration * SourceValue(1 / u.user_journey, label="One request per user session"),
    request_duration=request_duration,
    cpu_needed=NB_OF_GPUs * cpu_core_per_gpu_uj)
...
graph = build_calculus_graph(server.instances_fabrication_footprint)
graph.show(os.path.join(os.path.abspath(os.path.dirname(__file__)), "server_fab_calculation_graph.html"))
